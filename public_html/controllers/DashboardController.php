...
final class DashboardController extends BaseController
{
    public function index(): void
    {
        ...
        $summarySql = "SELECT
            ...
            FROM (
                SELECT 'payable' kind, remaining_amount amount FROM payables WHERE due_date BETWEEN ? AND ? AND status IN ('pendente','vencido','parcial')
                UNION ALL
                SELECT 'receivable', remaining_amount FROM receivables WHERE due_date BETWEEN ? AND ? AND status IN ('pendente','vencido','parcial')
            ) x";
        ...
        $cards = [];
        foreach (['payables' => ['payables', 'remaining_amount'], 'receivables' => ['receivables', 'remaining_amount']] as $key => [$table, $amount]) {
        ...
        $cashStmt = $pdo->query("SELECT day,
            ...
            UNION ALL SELECT due_date, 0, -remaining_amount FROM payables WHERE status IN ('pendente','vencido','parcial') AND due_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)
            UNION ALL SELECT due_date, 0, remaining_amount FROM receivables WHERE status IN ('pendente','vencido','parcial') AND due_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)
        ...
        $alerts = $pdo->query("SELECT 'Saída' kind, id, description, due_date, remaining_amount amount FROM payables WHERE status IN ('vencido','pendente','parcial') AND due_date <= CURDATE()
            UNION ALL SELECT 'Entrada', id, description, due_date, remaining_amount FROM receivables WHERE status IN ('vencido','pendente','parcial') AND due_date <= CURDATE()

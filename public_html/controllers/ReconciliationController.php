...
final class ReconciliationController extends BaseController
{
    public function index(): void
    {
        ...
        $stmt=db()->prepare("SELECT due_date,kind,description,expected,realized,status FROM (
            SELECT due_date,'Saída' kind,description,-(amount-paid_amount) expected,-paid_amount realized,status FROM payables WHERE status<>'cancelado' AND due_date BETWEEN ? AND ?
            UNION ALL
            SELECT due_date,'Entrada',description,(expected_amount-received_amount),received_amount,status FROM receivables WHERE status<>'cancelado' AND due_date BETWEEN ? AND ?

...
<form class="card mb-5 grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-9">
  <input type="hidden" name="route" value="receivables">
  <div class="relative sm:col-span-2 xl:col-span-2"><i data-lucide="search" class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"></i><input class="field !pl-10" type="search" name="q" value="<?=e($search)?>" maxlength="100" aria-label="Pesquisar contas a receber" placeholder="Pesquisar descrição, cliente, CPF/CNPJ, categoria ou observação"></div>
  <select class="field" name="status"><option value="">Status</option><?php foreach (['pendente' => 'Pendente', 'vencido' => 'Vencido', 'parcial' => 'Parcial', 'recebido' => 'Recebido', 'cancelado' => 'Cancelado'] as $v => $l): ?><option value="<?=$v?>" <?=($_GET['status'] ?? '') === $v ? 'selected' : ''?>><?=$l?></option><?php endforeach; ?></select>
  ...
  <select class="field" name="contact"><option value="">Cliente</option><?php foreach ($contacts as $o): ?><option value="<?=$o['id']?>" <?=($_GET['contact'] ?? '') == $o['id'] ? 'selected' : ''?>><?=e($o['name'])?></option><?php endforeach; ?></select>
  <select class="field" name="payment_method"><option value="">Forma</option><?php foreach (['pix' => 'Pix', 'cartao_credito' => 'Cartão crédito', 'cartao_debito' => 'Cartão débito', 'boleto' => 'Boleto', 'dinheiro' => 'Dinheiro', 'transferencia' => 'Transferência'] as $v => $l): ?><option value="<?=$v?>" <?=($_GET['payment_method'] ?? '') === $v ? 'selected' : ''?>><?=$l?></option><?php endforeach; ?></select>
...
<div class="card table-wrap">
  <table class="data-table">
    <thead><tr><th>Situação</th><th>Descrição</th><th>Vencimento</th><th>Total</th><th>Recebido</th><th>Restante</th><th>Ações</th></tr></thead>
    <tbody>
      ...
      ?>
        <tr>
          ...
          <td class="font-medium"><?=e($i['description'])?><span class="block text-xs font-normal text-slate-500"><?=e($i['contact_name'] ?: 'Sem cliente')?> · <?=e($i['category_name'] ?: 'Sem categoria')?></span></td>
          ...
          <td>
            <div class="relative inline-block">
              ...
              <div class="fixed z-50 hidden w-52 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl dark:border-slate-700 dark:bg-slate-900" data-action-menu>
                ...
                <?php if (!in_array($i['status'], ['recebido', 'cancelado'], true) && (float) $i['remaining_amount'] > 0): ?><button type="button" class="receivable-receive flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-800" data-id="<?=$i['id']?>" data-description="<?=e($i['description'])?>" data-remaining="<?=e($i['remaining_amount'])?>"><i data-lucide="banknote"></i>Baixar</button><?php endif; ?>
      ...
      <?php endforeach; ?>
      <?php if (!$items): ?><tr><td colspan="7" class="text-center text-slate-500">Nenhuma conta encontrada.</td></tr><?php endif; ?>
    </tbody>
  </table>
</div>

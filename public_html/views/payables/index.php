            ...
            <div class="relative inline-block">
              ...
              <div class="fixed z-50 hidden w-52 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl dark:border-slate-700 dark:bg-slate-900" data-action-menu>
                ...
                <?php if (!in_array($i['status'], ['pago', 'cancelado'], true) && (float) $i['remaining_amount'] > 0): ?>
                  <button type="button" class="payable-pay flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-800" data-id="<?=$i['id']?>" data-description="<?=e($i['description'])?>" data-remaining="<?=e($i['remaining_amount'])?>" data-method="<?=e($i['payment_method'])?>"><i data-lucide="banknote"></i>Baixar</button>
                <?php endif; ?>
...
(() => {
  ...
  const payId=document.getElementById('payId'),payDescription=document.getElementById('payDescription'),payRemaining=document.getElementById('payRemaining'),payAmount=document.getElementById('payAmount'),payMethod=document.getElementById('payMethod');
